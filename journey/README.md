...
## Known contracts & gaps (2026-08-18, from UI spec authoring + dry run)
...
- **Event payload shapes** are constrained per type in `event.schema.json` (e.g.,
  `claim.verdict` requires `verdict|tier|record`; `quiz.attempted` requires
  `bank|score|bloom_scores`; `matrix.updated` requires `level|score|validated`).
...
## Validating a journey root (including test journeys)

`python tools/validate-journey.py [root]` validates profile, state files, and every
JSONL log line against the committed schemas. Default root: `.journey/`. Test journeys
(e.g. dry runs) live under `workspace/` and are validated explicitly:
`python tools/validate-journey.py workspace/dryrun-journey`.
