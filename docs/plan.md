...
Wave 3: `data` core, `quality-testing` core, `security` core, `ai-ml` core, `frontiers` (T4).
**Wave 3 status: COMPLETED 2026-08-18** — 23 packs (data 5, quality 5, security 5, ai-ml 4, frontiers 4) drafted by 12 L1 agents (batched due to interruptions), L2 review by `deepseek/deepseek-v4-pro` (report: `docs/reviews/l2-wave3-review-2026-08-18.md`; 16 pass, 2 pass-with-fixes, 5 fail), fixes applied: systematic T0 over-grading corrected by the record-tier rule (5 packs downgraded: sql→T1, crypto→T2, ml-fundamentals→T1, supervised→T1, neural→T1), Vaswani duplicate consolidated (S-0269/S-0272), B-tree teaching bug, bloom_target markers, PLONK preprint re-typed. Published: 56/68 topics. Remaining 12 draft scaffolds = wave 4 (cs-foundations + programming + engineering + hardware leftovers). Coverage 16/311 (5%).
